...
def test_release_layout_windows(tmp_path):
    ...
    assert '安装并启动 Lulu.cmd' in readme and 'SmartScreen' in readme and '%LOCALAPPDATA%\\Lulu\\logs\\desktop.log' in readme and '{' not in readme and '轻量版' not in readme
...
def test_release_layout_macos_lite_experiment_with_runtime(tmp_path):
    ...
    readme = (folder / '先看这里.md').read_text(encoding='utf-8')
    assert '轻量版' in readme and '不需要安装 Python' in readme and '仍要打开' in readme
