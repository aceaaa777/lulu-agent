...
def run(godot, args, project=PROJECT):
...
def main():
    ...
    parser.add_argument('--project', type=Path, default=PROJECT, help='Godot 项目目录（英文包用 build/src-en/desktop）')
    ...
    if not (project / '.godot' / 'imported').exists():
        ...
        code, errors = run(godot, ['--editor', '--import', '--quit'], project)
        if errors:
            print('\n'.join(errors[:10]))
    ...
    for key in [t.strip() for t in args.targets.split(',') if t.strip()]:
        ...
        code, errors = run(godot, [mode, preset, str(target)], project)
