...
func set_agent_status(value: String):
	...
	if value in ['thinking','blocked','working']:
		...
		feedback('resolved' if recovered else 'thinking_done');recovered=false
	elif value=='cancelled':
		completion_sign=false;recovered=false
...
func _feedback_end() -> bool:
	if phase in ['thinking_enter','blocked_enter']:
		...
		if completion_sign and not quitting:pending_feedback='complete';completion_sign=false
		_route()
