...
func make_panel():
	...
	look_label.size_flags_vertical=Control.SIZE_SHRINK_CENTER; ui_scale_option.size_flags_vertical=Control.SIZE_SHRINK_CENTER
	var look_hint=label(look,'字太小就调大一档，窗口里的字和按钮一起放大。'); look_hint.add_theme_font_size_override('font_size',14); look_hint.add_theme_color_override('font_color',Color('#7d6e5d')); look_hint.size_flags_horizontal=Control.SIZE_EXPAND_FILL; look_hint.size_flags_vertical=Control.SIZE_SHRINK_CENTER
...
func poll():
	...
	animation.english=str(response.get('lang',''))=='en'
...
func _process(delta):
	...
	elif phase in ['enter','work','exit']:pet_status.text='在做了，噼里啪啦～'
