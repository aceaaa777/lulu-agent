...
def _memory_key_text(key):
    """A memory's name: Chinese keys are a few characters without punctuation; English keys keep their spaces (2–4 words)."""
    if LANG == 'en':
        return re.sub(r'\s+', ' ', re.sub(r'[^\w\s\-]', '', key)).strip()[:40]
    return re.sub(r'[^\w\u4e00-\u9fff]', '', key)[:12]
...
async def memory_key(run, content):
    ...
    try:
        ...
        key = ''
    key = _memory_key_text(key)
    return key or content[:(40 if LANG == 'en' else 8)]


CANCEL_CUE_EN = r"\b(?:cancel|delete|remove|drop|clear)\b.{0,20}\b(?:reminder|alarm)s?\b|\b(?:reminder|alarm)s?\b.{0,12}\b(?:cancel|delete|remove)\b"
LIST_CUE_EN = r"\b(?:list|show|see|view|what|which|any)\b.{0,24}\b(?:reminder|alarm)s?\b|\bmy (?:reminder|alarm)s\b"
TIME_SCHEMA = {'title': 'when', 'type': 'object', 'additionalProperties': False,
               'properties': {'date': {'type': 'string'}, 'time': {'type': 'string'}, 'repeat': {'type': 'string', 'enum': ['none', 'hourly', 'daily', 'weekly']},
                              'task': {'type': 'string'}}, 'required': ['date', 'time', 'repeat', 'task']}


async def model_parse_time(run, text, now=None):
    """English build only: when the time rules find nothing, let the model read the request into a date/time/repeat
    and the program does the arithmetic; the reply echoes the parsed time so a wrong reading is visible at once."""
    now = now or datetime.now()
    system = (f'Now is {now:%A, %Y-%m-%d %H:%M}. Read the reminder request and output JSON only: date as YYYY-MM-DD, time as 24-hour HH:MM '
              '(09:00 if no time is stated), repeat as none/hourly/daily/weekly, and task = the request without the time words. '
              'Never invent a date that is not implied by the request.')
    reply = await run.provider.chat([{'role': 'system', 'content': system}, {'role': 'user', 'content': text[:300]}], schema=TIME_SCHEMA,
                                    max_tokens=120, temperature=0, timeout=run.budget.step_timeout(500, 120))
    if reply.failed:
        return None
    try:
        data = json.loads(reply.content)
        due = datetime.strptime(str(data.get('date', ''))[:10]+' '+str(data.get('time', '09:00'))[:5], '%Y-%m-%d %H:%M')
    except ValueError:
        return None
    interval = {'hourly': 3600, 'daily': 86400, 'weekly': 604800}.get(str(data.get('repeat', 'none')), 0)
    ...
    label = due.strftime('on %a %m/%d at %H:%M')+(', every '+{3600: 'hour', 86400: 'day', 604800: 'week'}[interval] if interval else '')
    task = re.sub(r'\s+', ' ', str(data.get('task', ''))).strip(' ,.') or "It's time"
    return {'due': due.timestamp(), 'interval': interval, 'text': task, 'when': label, 'by_model': True}


async def remind(run):
    text = positive(run.text)
    if re.search(r'(?:取消|删除|删掉).{0,4}提醒', text) or (LANG == 'en' and re.search(CANCEL_CUE_EN, text, re.I)):
    ...
    if re.search(r'(?:查看|列出|有哪些|有什么).{0,4}提醒', text) or (LANG == 'en' and re.search(LIST_CUE_EN, text, re.I)):
        pending = run.store.rows("SELECT * FROM reminders WHERE status='pending' ORDER BY due")
        if not pending:
            return ('completed', '暂时没有要提醒你的事。')
    ...
    if not parsed and LANG == 'en' and not when_text:
        parsed = await model_parse_time(run, text)
...
async def memory_candidates(run):
    ...
    for c in items if isinstance(items, list) else []:
        ...
        key = _memory_key_text(str(c.get('key', '')))
