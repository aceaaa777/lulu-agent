...
# 桌宠动作：任务一旦进入“动手”阶段（技能开始、翻译/改写/提取开始、起草、上网查、跑工具、返工重做）就敲键盘；
# 只在分类、组织回答、等模型这类纯“想”的阶段才做思考动作。以任务最新一条事件为准，服务端是唯一裁判。
WORKING_KINDS={'skill_started','translate_started','rewrite_started','extract_started','drafting','tool_started','tool_done',
               'research_started','search_started','search_results','pages_fetched','facts_verified','evidence_collected','network_evidence',
               'generate_retry','translate_retry','rewrite_retry','summary_retry','draft_retry','generate_inline','answer_regrounded','follow_up','step_reused'}
def _detail(raw):
    try: return json.loads(raw) if isinstance(raw,str) else raw
    except ValueError: return None
THINKING_SKILLS={'chat','memory_qa'}   # 聊天、翻记忆回答：没有“动手”的部分，整段都是想
def animation_for(kind, detail=None):
    ...
    async def desktop(request):
        ...
        for task in tasks:
            if task['status'] in ('queued','running'):
                events=store.rows("SELECT kind,detail FROM events WHERE task=? ORDER BY id DESC LIMIT 1",(task['id'],))
                task['animation']=animation_for(events[0]['kind'],_detail(events[0]['detail'])) if events else 'thinking'
        provider=agent.provider
        ...
        return web.json_response({'tasks':tasks,'questions':store.rows("SELECT q.* FROM questions q JOIN tasks t ON t.id=q.task WHERE q.status='pending' AND t.status NOT IN ('queued','running') ORDER BY q.created"),'reminders':store.rows("SELECT * FROM reminders WHERE status='pending' ORDER BY due"),'now':time.time(),'lang':LANG,'backend':backend,'last_artifact':last_artifact})
