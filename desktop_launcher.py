...
def package_root(start):
    ...
    for folder in [start,*start.parents]:
        # exact, case-sensitive names: on Windows (folder/'desktop').is_dir() is also true for C:\\Users\\<name>\\Desktop
        try: names={entry.name for entry in os.scandir(folder) if entry.is_dir()}
        except OSError: names=set()
        if 'pet' in names or ('desktop' in names and (folder/'desktop'/'project.godot').exists()): return folder
    return start
...
def find_pet(root):
    ...
    wanted={'win32':'.exe','cygwin':'.exe','darwin':'Lulu.app'}.get(sys.platform,'.x86_64')   # sys.platform only, so tests can pretend
